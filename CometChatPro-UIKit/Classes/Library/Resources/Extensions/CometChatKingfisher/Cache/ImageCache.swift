...
extension Notification.Name {
    ...
    /// `CometChatKingfisherDiskCacheCleanedHashKey` key in `userInfo` of the notification object you received.
    /// By checking the array, you could know the hash codes of files are removed.
    public static let CometChatKingfisherDidCleanDiskCache =
        Notification.Name("com.onevcat.CometChatKingfisher.CometChatKingfisherDidCleanDiskCache")
}

/// Key for array of cleaned hashes in `userInfo` of `CometChatKingfisherDidCleanDiskCacheNotification`.
public let CometChatKingfisherDiskCacheCleanedHashKey = "com.onevcat.CometChatKingfisher.cleanedHash"
...
public struct CacheStoreResult {
    ...
    public let diskCacheResult: Result<(), CometChatKingfisherError>
}

extension CFCrossPlatformImage: CacheCostCalculable {
    /// Cost of an image
    public var cacheCost: Int { return cf.cost }
}
...
public enum ImageCacheResult {
    ...
    case disk(CFCrossPlatformImage)
    
    /// The image can be retrieved memory cache.
    case memory(CFCrossPlatformImage)
    ...
    public var image: CFCrossPlatformImage? {
        switch self {
...
///
/// While a default image cache object will be used if you prefer the extension methods of CometChatKingfisher, you can create
...
open class ImageCache {
    ...
    // MARK: Singleton
    /// The default `ImageCache` object. CometChatKingfisher will use this cache for its related methods if there is no
    ...
    public let memoryStorage: MemoryStorage.Backend<CFCrossPlatformImage>
    ...
    public init(
        memoryStorage: MemoryStorage.Backend<CFCrossPlatformImage>,
        diskStorage: DiskStorage.Backend<Data>)
    {
        self.memoryStorage = memoryStorage
        self.diskStorage = diskStorage
        let ioQueueName = "com.onevcat.CometChatKingfisher.ImageCache.ioQueue.\(UUID().uuidString)"
    ...
    {
        if name.isEmpty {
            fatalError("[CometChatKingfisher] You should specify a name for the cache. A cache with empty name is not permitted.")
        }

        let totalMemory = ProcessInfo.processInfo.physicalMemory
        let costLimit = totalMemory / 4
        let memoryStorage = MemoryStorage.Backend<CFCrossPlatformImage>(config:
                    ...
                    completionHandler: ((CacheStoreResult) -> Void)? = nil)
    {
        ...
        ioQueue.async {
            ...
            if let data = serializer.data(with: image, original: original) {
                ...
                let diskError = CometChatKingfisherError.cacheError(
    ...
    ///               further use. By default, CometChatKingfisher uses a `DefaultCacheSerializer` to serialize the image to
        ...
        completionHandler: ((CacheStoreResult) -> Void)? = nil)
    {
        ...
        do {
            ...
            let diskError: CometChatKingfisherError
            if let error = error as? CometChatKingfisherError {
                diskError = error
            } else {
    ...
    func retrieveImage(forKey key: String,
                       ...
                       completionHandler: ((Result<ImageCacheResult, CometChatKingfisherError>) -> Void)?)
    {
    ...
    /// - Parameters:
    ///   - key: The key used for caching the image.
    ///   - options: The `CometChatKingfisherOptionsInfo` options setting used for retrieving the image.
    ...
    ///                        will be sent to this closure as result. Otherwise, a `CometChatKingfisherError` result
                     ...
                     completionHandler: ((Result<ImageCacheResult, CometChatKingfisherError>) -> Void)?)
    {
        retrieveImage(
            forKey: key,
            options: CometChatKingfisherParsedOptionsInfo(options),
            callbackQueue: callbackQueue,
            completionHandler: completionHandler)
    }
    ...
    func retrieveImageInMemoryCache(
        forKey key: String,
        options: CometChatKingfisherParsedOptionsInfo) -> CFCrossPlatformImage?
    {
    ...
    open func retrieveImageInMemoryCache(
        forKey key: String,
        options: CometChatKingfisherOptionsInfo? = nil) -> CFCrossPlatformImage?
    {
        return retrieveImageInMemoryCache(forKey: key, options: CometChatKingfisherParsedOptionsInfo(options))
    }
    ...
    func retrieveImageInDiskCache(
        ...
        options: CometChatKingfisherParsedOptionsInfo,
        callbackQueue: CallbackQueue = .untouch,
        completionHandler: @escaping (Result<CFCrossPlatformImage?, CometChatKingfisherError>) -> Void)
    {
        let computedKey = key.computedKey(with: options.processor.identifier)
        let loadingQueue: CallbackQueue = options.loadDiskFileSynchronously ? .untouch : .dispatch(ioQueue)
        loadingQueue.execute {
            do {
                var image: CFCrossPlatformImage? = nil
                ...
                if let error = error as? CometChatKingfisherError {
                    ...
                    assertionFailure("The internal thrown error should be a `CometChatKingfisherError`.")
                }
            }
        }
    }
                ...
                if !removed.isEmpty {
                    DispatchQueue.main.async {
                        ...
                        NotificationCenter.default.post(
                            name: .CometChatKingfisherDidCleanDiskCache,
                            object: self,
                            userInfo: [CometChatKingfisherDiskCacheCleanedHashKey: cleanedHashes])
                    }
                }
    ...
    @objc public func backgroundCleanExpiredDiskCache() {
        ...
        guard let sharedApplication = CometChatKingfisherWrapper<UIApplication>.shared else { return }
    ...
    open func calculateDiskStorageSize(completion handler: @escaping ((Result<UInt, CometChatKingfisherError>) -> Void)) {
        ioQueue.async {
            do {
...
extension UIApplication: CometChatKingfisherCompatible { }
extension CometChatKingfisherWrapper where Base: UIApplication {
