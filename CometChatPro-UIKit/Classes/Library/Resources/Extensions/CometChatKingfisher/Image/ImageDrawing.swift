...
extension CometChatKingfisherWrapper where Base: CFCrossPlatformImage {
    ...
    public func image(withBlendMode blendMode: CGBlendMode,
                      alpha: CGFloat = 1.0,
                      backgroundColor: CFCrossPlatformColor? = nil) -> CFCrossPlatformImage
    {
        guard let _ = cgImage else {
            assertionFailure("[CometChatKingfisher] Blend mode image only works for CG-based image.")
            return base
        }
    ...
    {
        guard let _ = cgImage else {
            assertionFailure("[CometChatKingfisher] Compositing Operation image only works for CG-based image.")
            return base
        }
    ...
    {
        guard let _ = cgImage else {
            assertionFailure("[CometChatKingfisher] Round corner image only works for CG-based image.")
            return base
        }
        ...
        return draw(to: size) { _ in
            ...
            guard let context = UIGraphicsGetCurrentContext() else {
                assertionFailure("[CometChatKingfisher] Failed to create CG context for image.")
                return false
            }
    ...
    #if os(iOS) || os(tvOS)
    func resize(to size: CGSize, for contentMode: UIView.ContentMode) -> CFCrossPlatformImage {
        switch contentMode {
        case .scaleAspectFit:
    ...
    /// - Note: This method only works for CG-based image. The current image scale is kept.
    ///         For any non-CG-based image, `base` itself is returned.
    public func resize(to size: CGSize) -> CFCrossPlatformImage {
        guard let _ = cgImage else {
            assertionFailure("[CometChatKingfisher] Resize only works for CG-based image.")
            return base
        }
    ...
    /// - Note: This method only works for CG-based image. The current image scale is kept.
    ///         For any non-CG-based image, `base` itself is returned.
    public func resize(to targetSize: CGSize, for contentMode: ContentMode) -> CFCrossPlatformImage {
        let newSize = size.cf.resize(to: targetSize, for: contentMode)
    ...
    /// - Note: This method only works for CG-based image. The current image scale is kept.
    ///         For any non-CG-based image, `base` itself is returned.
    public func crop(to size: CGSize, anchorOn anchor: CGPoint) -> CFCrossPlatformImage {
        guard let cgImage = cgImage else {
            assertionFailure("[CometChatKingfisher] Crop only works for CG-based image.")
            return base
        }
        
        let rect = self.size.cf.constrainedRect(for: size, anchor: anchor)
        guard let image = cgImage.cropping(to: rect.scaled(scale)) else {
            assertionFailure("[CometChatKingfisher] Cropping image failed.")
            return base
        }
        
        return CometChatKingfisherWrapper.image(cgImage: image, scale: scale, refImage: base)
    }
    ...
    /// - Note: This method only works for CG-based image. The current image scale is kept.
    ///         For any non-CG-based image, `base` itself is returned.
    public func blurred(withRadius radius: CGFloat) -> CFCrossPlatformImage {
        
        guard let cgImage = cgImage else {
            assertionFailure("[CometChatKingfisher] Blur only works for CG-based image.")
            return base
        }
        ...
        // See https://github.com/onevcat/CometChatKingfisher/issues/611
        ...
        guard let context = beginContext(size: size, scale: scale, inverting: true) else {
            assertionFailure("[CometChatKingfisher] Failed to create CG context for blurring image.")
            return base
        }
        ...
        #else
        let result = outContext.makeImage().flatMap {
            CFCrossPlatformImage(cgImage: $0, scale: base.scale, orientation: base.imageOrientation)
        }
        #endif
        guard let blurredImage = result else {
            assertionFailure("[CometChatKingfisher] Can not make an blurred image within this context.")
            return base
        }
    ...
    /// - Note: This method only works for CG-based image. The current image scale is kept.
    ///         For any non-CG-based image, `base` itself is returned.
    public func overlaying(with color: CFCrossPlatformColor, fraction: CGFloat) -> CFCrossPlatformImage {
        
        guard let _ = cgImage else {
            assertionFailure("[CometChatKingfisher] Overlaying only works for CG-based image.")
            return base
        }
    ...
    public func tinted(with color: CFCrossPlatformColor) -> CFCrossPlatformImage {
        #if os(watchOS)
        return base
        #else
    ...
    public func adjusted(brightness: CGFloat, contrast: CGFloat, saturation: CGFloat, inputEV: CGFloat) -> CFCrossPlatformImage {
        #if os(watchOS)
        return base
        #else
    ...
    public func scaled(to scale: CGFloat) -> CFCrossPlatformImage {
        guard scale != self.scale else {
            return base
        }
        guard let cgImage = cgImage else {
            assertionFailure("[CometChatKingfisher] Scaling only works for CG-based image.")
            return base
        }
        return CometChatKingfisherWrapper.image(cgImage: cgImage, scale: scale, refImage: base)
    }
}
...
extension CometChatKingfisherWrapper where Base: CFCrossPlatformImage {
    ...
    public var decoded: CFCrossPlatformImage { return decoded(scale: scale) }
    ...
    /// - Note: This method only works for CG-based image. The current image scale is kept.
    ///         For any non-CG-based image or animated image, `base` itself is returned.
    public func decoded(scale: CGFloat) -> CFCrossPlatformImage {
        ...
        guard let imageRef = cgImage else {
            assertionFailure("[CometChatKingfisher] Decoding only works for CG-based image.")
            return base
        }
...
extension CometChatKingfisherWrapper where Base: CFCrossPlatformImage {
    ...
    func beginContext(size: CGSize, scale: CGFloat, inverting: Bool = false) -> CGContext? {
        ...
        guard let rep = NSBitmapImageRep(
            ...
            bitsPerPixel: 0) else
        {
            assertionFailure("[CometChatKingfisher] Image representation cannot be created.")
            return nil
        }
        rep.size = size
        NSGraphicsContext.saveGraphicsState()
        guard let context = NSGraphicsContext(bitmapImageRep: rep) else {
            assertionFailure("[CometChatKingfisher] Image context cannot be created.")
            return nil
        }
    ...
    ) -> CFCrossPlatformImage
    {
        ...
        return CometChatKingfisherWrapper.image(cgImage: cgImage, scale: targetScale, refImage: ref)
    }
    
    #if os(macOS)
    func fixedForRetinaPixel(cgImage: CGImage, to size: CGSize) -> CFCrossPlatformImage {
        
        let image = CFCrossPlatformImage(cgImage: cgImage, size: base.size)
        let rect = CGRect(origin: CGPoint(x: 0, y: 0), size: size)
