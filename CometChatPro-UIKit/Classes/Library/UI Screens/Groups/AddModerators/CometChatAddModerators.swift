...
public class CometChatAddModerators: UIViewController {
    ...
    private func setupNavigationBar() {
        if navigationController != nil{
            ...
            switch mode {
            case .fetchGroupMembers: self.set(title: NSLocalizedString("MAKE_GROUP_MODERATOR", tableName: nil, bundle: CCPType.bundle, value: "", comment: ""), mode: .automatic)
            case .fetchModerators: self.set(title: NSLocalizedString("Moderators", tableName: nil, bundle: CCPType.bundle, value: "", comment: ""), mode: .automatic)
            case .none: break
            }
    ...
    private func fetchModerators(for group: Group){
        ...
        memberRequest?.fetchNext(onSuccess: { (groupMembers) in
            ...
            print("fetchModerators error:\(String(describing: error?.errorDescription))")
        })
    }
}
    ...
    public func tableView(_ tableView: UITableView, contextMenuConfigurationForRowAt indexPath: IndexPath, point: CGPoint) -> UIContextMenuConfiguration? {
        ...
        return UIContextMenuConfiguration(identifier: nil, previewProvider: nil, actionProvider: { suggestedActions in
            ...
            if self.mode == .fetchModerators {
                if indexPath.section == 0 &&  indexPath.row == 0 {
                    ...
                    if  let selectedCell = tableView.cellForRow(at: indexPath) as? MembersView {
                        if let member = selectedCell.member {
                            let removeAdmin = UIAction(title: NSLocalizedString("DISMISS_AS_MODERATOR", tableName: nil, bundle: CCPType.bundle, value: "", comment: ""), image: UIImage(systemName: "trash"), attributes: .destructive) { action in
                                ...
                                CometChat.updateGroupMemberScope(UID: member.uid ?? "", GUID: self.currentGroup?.guid ?? "", scope: .participant, onSuccess: { (success) in
                                    ...
                                    DispatchQueue.main.async {
                                        let message =  (member.name ?? "") + NSLocalizedString("is removed from admin privilege.", tableName: nil, bundle: CCPType.bundle, value: "", comment: "")
                            ...
                            if self.currentGroup?.owner == LoggedInUser.uid || self.currentGroup?.scope == .admin || self.currentGroup?.scope == .moderator {
                                ...
                                if member.uid != LoggedInUser.uid {
                                    return UIMenu(title: NSLocalizedString("REMOVE", comment: "") + "\(memberName)" + NSLocalizedString("AS_MODERATOR_FROM", comment: "") + "\(groupName)" + NSLocalizedString("GROUP?" , tableName: nil, bundle: CCPType.bundle, value: "", comment: ""), children: [removeAdmin])
                                }
                            }
                        }
                    }
                ...
                if  let selectedCell = tableView.cellForRow(at: indexPath) as? MembersView {
                    if let member = selectedCell.member {
                        let removeAdmin = UIAction(title: NSLocalizedString("ASSIGN_AS_MODERATOR", tableName: nil, bundle: CCPType.bundle, value: "", comment: ""), image: UIImage(systemName: "add"), attributes: .destructive) { action in
                            ...
                            CometChat.updateGroupMemberScope(UID: member.uid ?? "", GUID: self.currentGroup?.guid ?? "", scope: .moderator, onSuccess: { (success) in
                                ...
                                DispatchQueue.main.async {
                                    let message =  (member.name ?? "") + NSLocalizedString("IS_NOW_MODERATOR", tableName: nil, bundle: CCPType.bundle, value: "", comment: "")
                                    let snackbar: CometChatSnackbar = CometChatSnackbar.init(message: message, duration: .short)
                                    snackbar.show()
                                    if let group = self.currentGroup {
                                        self.fetchModerators(for: group)
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if mode == .fetchModerators {
            ...
            if #available(iOS 13.0, *) {
                ...
                if  let selectedCell = tableView.cellForRow(at: indexPath) as? MembersView {
                    if let member = selectedCell.member {
                        let alert = UIAlertController(title: NSLocalizedString("REMOVE", tableName: nil, bundle: CCPType.bundle, value: "", comment: ""), message: "Remove \(String(describing: member.name!.capitalized))" + NSLocalizedString(
                        "AS_A_MODERATOR", comment: ""), preferredStyle: .alert)
                        
                        alert.addAction(UIAlertAction(title: NSLocalizedString("OK", tableName: nil, bundle: CCPType.bundle, value: "", comment: ""), style: .default, handler: { action in
                            
                            CometChat.updateGroupMemberScope(UID: member.uid ?? "", GUID: self.currentGroup?.guid ?? "", scope: .participant, onSuccess: { (success) in
                                let message =  (member.name ?? "") + NSLocalizedString("REMOVE_FROM_MODERATOR_PREVILEDGE", tableName: nil, bundle: CCPType.bundle, value: "", comment: "")
                                let snackbar: CometChatSnackbar = CometChatSnackbar.init(message: message, duration: .short)
                                snackbar.show()
                                DispatchQueue.main.async {
                                    if let group = self.currentGroup {
            ...
            if #available(iOS 13.0, *) {
                ...
                if  let selectedCell = tableView.cellForRow(at: indexPath) as? MembersView {
                    ...
                    if let member = selectedCell.member {
                        let alert = UIAlertController(title: NSLocalizedString("ADD" , tableName: nil, bundle: CCPType.bundle, value: "", comment: ""), message: "Add \(String(describing: member.name!.capitalized))" + NSLocalizedString(
                        "AS_A_MODERATOR", comment: ""), preferredStyle: .alert)
