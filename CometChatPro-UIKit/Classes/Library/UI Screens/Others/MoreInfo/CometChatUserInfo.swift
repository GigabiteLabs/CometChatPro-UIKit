...
public class CometChatUserInfo: UIViewController {
     ...
     * .always: Always use a larger title when this item is topmost.
     ...
     - Copyright:  ©  2020 CometChat Inc.
     */
    public func updateUserInformation(withName: String?, config: CCPConfig) {
        ...
        CometChat.updateUser(user: user, apiKey: config.apiKey, onSuccess: { (user) in
              DispatchQueue.main.async {
...
extension CometChatUserInfo: CometChatUserViewDelegate {
    ...
    func didEditInfoPressed(config: CCPConfig) {
        ...
        let save = UIAlertAction(title: "Update", style: .default) { (alertAction) in
            ...
            if textField.text != "" {
                self.updateUserInformation(withName: textField.text, config: CCPConfig.shared)
            } else {
