...
public class CometChatMessageList: UIViewController, AVAudioRecorderDelegate, AVAudioPlayerDelegate, UIGestureRecognizerDelegate {
    ...
    @objc public func set(conversationWith: AppEntity, type: CometChat.ReceiverType){
        ...
        case .user:
            ...
            case .online:
                ...
                setupNavigationBar(withSubtitle: NSLocalizedString("ONLINE", tableName: nil, bundle: CCPType.bundle, value: "", comment: ""))
                setupNavigationBar(withImage: user.avatar ?? "", name: user.name ?? "", bool: true)
            case .offline:
                setupNavigationBar(withTitle: user.name?.capitalized ?? "")
                setupNavigationBar(withSubtitle: NSLocalizedString("OFFLINE", tableName: nil, bundle: CCPType.bundle, value: "", comment: ""))
                setupNavigationBar(withImage: user.avatar ?? "", name: user.name ?? "", bool: true)
            default:
                break
            }
    ...
    public func set(liveReaction: Reaction) {
        ...
        case .heart:
            ...
            reaction.setImage(.fromBundle(named: "heart"), for: .normal)
            reactionView.image1 = .fromBundle(named: "heart")
        case .thumbsup:
            self.currentReaction = liveReaction
            reaction.setImage(.fromBundle(named: "thumbsup"), for: .normal)
            reactionView.image1 = .fromBundle(named: "thumbsup")
        }
    }
            ...
            if  user?.blockedByMe == true {
                if let name = strongSelf.currentUser?.name {
                    DispatchQueue.main.async {
                        ...
                        strongSelf.blockedMessage.text = NSLocalizedString("YOU'VE_BLOCKED", tableName: nil, bundle: CCPType.bundle, value: "", comment: "") + " \(String(describing: name.capitalized))"
                    }
                }
            }
        }) { (error) in
    ...
    private func fetchGroup(group: String){
        CometChat.getGroup(GUID: group, onSuccess: { [weak self] (group) in
            ...
            if  group.membersCount == 1 {
                ...
                strongSelf.setupNavigationBar(withTitle: group.name?.capitalized ?? "")
                strongSelf.setupNavigationBar(withSubtitle: "\(group.membersCount)" + NSLocalizedString("MEMBERS", tableName: nil, bundle: CCPType.bundle, value: "", comment: ""))
                strongSelf.membersCount = "\(group.membersCount) " + NSLocalizedString("MEMBERS", tableName: nil, bundle: CCPType.bundle, value: "", comment: "")
            }
        }) { (error) in
    ...
    private func didExtensionDetected(message: BaseMessage) -> CometChatExtension {
        ...
        if let metaData = message.metaData , let type = metaData["type"] as? String {
            if type == "reply" {
                ...
                detectedExtension = CometChatExtension.none
            }
    ...
    @objc func didUserBlocked(_ notification: NSNotification) {
        if let name = notification.userInfo?["name"] as? String {
            ...
            blockedMessage.text =
                NSLocalizedString("YOU'VE_BLOCKED", tableName: nil, bundle: CCPType.bundle, value: "", comment: "") + "\(String(describing: name.capitalized))"
        }
    }
    ...
    private func addBackButton(bool: Bool) {
        ...
        backButton.setImage(.fromBundle(named: "back"), for: .normal) // Image can be downloaded from here below link
        backButton.setTitle(NSLocalizedString("BACK", tableName: nil, bundle: CCPType.bundle, value: "", comment: ""), for: .normal)
        backButton.setTitleColor(backButton.tintColor, for: .normal) // You can change the TitleColor
        backButton.addTarget(self, action: #selector(self.didBackButtonPressed(_:)), for: .touchUpInside)
        
        let cancelButton = UIButton(type: .custom)
        cancelButton.setTitle(NSLocalizedString("CANCEL", tableName: nil, bundle: CCPType.bundle, value: "", comment: ""), for: .normal)
    ...
    private func setupNavigationBar(withImage URL: String, name: String, bool: Bool){
        DispatchQueue.main.async {
            ...
            if bool == true {
                ...
                let copyButton = UIBarButtonItem(image: .fromBundle(named: "copy"), style: .done, target: self, action: #selector(self.didCopyButtonPressed(_:)))
    ...
    @IBAction func didCopyButtonPressed(_ sender: UIButton) {
        if selectedMessages.isEmpty {
            DispatchQueue.main.async {
                let snackbar: CometChatSnackbar = CometChatSnackbar.init(message: NSLocalizedString("SELECT_A_MESSGE", tableName: nil, bundle: CCPType.bundle, value: "", comment: ""), duration: .short)
                snackbar.show()
            }
        }else{
            var messages = [String]()
            for message in selectedMessages {
                ...
                case .custom: messageText = NSLocalizedString("CUSTOM_MESSAGE", tableName: nil, bundle: CCPType.bundle, value: "", comment: "")
            ...
            DispatchQueue.main.async {
                let snackbar: CometChatSnackbar = CometChatSnackbar.init(message: NSLocalizedString("TEXT_COPIED", tableName: nil, bundle: CCPType.bundle, value: "", comment: ""), duration: .short)
                snackbar.show()
                self.didPreformCancel()
            }
        }
    ...
    private func registerCells() {
        ...
        tableView?.register(.RightImageMessageBubble)
        tableView?.register(.LeftVideoMessageBubble)
        tableView?.register(.RightVideoMessageBubble)
        tableView?.register(.LeftFileMessageBubble)
        tableView?.register(.RightFileMessageBubble)
        tableView?.register(.LeftAudioMessageBubble)
        tableView?.register(.RightAudioMessageBubble)
        tableView?.register(.ActionMessageBubble)
        tableView?.register(.LeftLinkPreviewBubble)
        tableView?.register(.RightLinkPreviewBubble)
        tableView?.register(.LeftReplyMessageBubble)
        tableView?.register(.RightReplyMessageBubble)
    }
...
extension CometChatMessageList: UIDocumentPickerDelegate {
    private func playOutgoingMessageSound(pausingCurrentAudio: Bool?) {
        DispatchQueue.main.async {
            do {
    ...
    private func playIncomingMessageSound(pausingCurrentAudio: Bool?) {
        DispatchQueue.main.async {
            do {
    ...
    private func playIncomingMessageOther(pausingCurrentAudio: Bool?) {
        DispatchQueue.main.async {
            do {
    ...
    public func documentPicker(_ controller: UIDocumentPickerViewController, didPickDocumentsAt urls: [URL]) {
        if controller.documentPickerMode == UIDocumentPickerMode.import {
            ...
            playOutgoingMessageSound(pausingCurrentAudio: true)
    ...
    public func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        if let firstMessageInSection = chatMessages[section].first {
            ...
            if dateString == "01/01/1970" {
                label.text = NSLocalizedString("TODAY", tableName: nil, bundle: CCPType.bundle, value: "", comment: "")
            }else{
        ...
        if let message = chatMessages[indexPath.section][safe: indexPath.row] {
            if message.messageCategory == .message {
                if message.deletedAt > 0.0 && message.senderUid != LoggedInUser.uid {
                    let  deletedCell = tableView.dequeueReusableCell(with: .LeftTextMessageBubble, for: indexPath) as! LeftTextMessageBubble
                    deletedCell.deletedMessage = message
                    return deletedCell
                    
                }else if message.deletedAt > 0.0 && message.senderUid == LoggedInUser.uid {
                    
                    let deletedCell = tableView.dequeueReusableCell(with: .RightTextMessageBubble, for: indexPath) as! RightTextMessageBubble
                    deletedCell.deletedMessage = message
                    ...
                    case .text where message.senderUid != LoggedInUser.uid:
                        if let textMessage = message as? TextMessage {
                            ...
                            case .linkPreview:
                                let receiverCell = tableView.dequeueReusableCell(with: .LeftLinkPreviewBubble, for: indexPath) as! LeftLinkPreviewBubble
                                ...
                                return receiverCell
                            case .reply:
                                let receiverCell = tableView.dequeueReusableCell(with: .LeftReplyMessageBubble, for: indexPath) as! LeftReplyMessageBubble
                            ...
                            case .smartReply,.messageTranslation, .profanityFilter, .sentimentAnalysis, .none:
                                let receiverCell = tableView.dequeueReusableCell(with: .LeftTextMessageBubble, for: indexPath) as! LeftTextMessageBubble
                                receiverCell.indexPath = indexPath
                                receiverCell.delegate = self
                                receiverCell.textMessage = textMessage
                                self.addSwipeGestureForMessage(cell: receiverCell, message: message)
                    ...
                    case .text where message.senderUid == LoggedInUser.uid:
                        if let textMessage = message as? TextMessage {
                            ...
                            switch isContainsExtension {
                            case .linkPreview:
                                let senderCell = tableView.dequeueReusableCell(with: .RightLinkPreviewBubble, for: indexPath) as! RightLinkPreviewBubble
                                let linkPreviewMessage = message as? TextMessage
                                ...
                                return senderCell
                            case .reply:
                                let senderCell = tableView.dequeueReusableCell(with: .RightReplyMessageBubble, for: indexPath) as! RightReplyMessageBubble
                            ...
                            case .smartReply,.messageTranslation, .profanityFilter, .sentimentAnalysis, .none:
                                let senderCell = tableView.dequeueReusableCell(with: .RightTextMessageBubble, for: indexPath) as! RightTextMessageBubble
                                senderCell.textMessage = textMessage
                                senderCell.indexPath = indexPath
                                self.addSwipeGestureForMessage(cell: senderCell, message: message)
                    ...
                    case .image where message.senderUid != LoggedInUser.uid:
                        ...
                        if let imageMessage = message as? MediaMessage {
                            ...
                            case .thumbnailGeneration, .imageModeration,.none:
                                let receiverCell = tableView.dequeueReusableCell(with: .LeftImageMessageBubble, for: indexPath) as! LeftImageMessageBubble
                    ...
                    case .image where message.senderUid == LoggedInUser.uid:
                        ...
                        if let imageMessage = message as? MediaMessage {
                            ...
                            case .linkPreview, .smartReply, .messageTranslation, .profanityFilter,.sentimentAnalysis, .reply: break
                            case .thumbnailGeneration, .imageModeration,.none:
                                let senderCell = tableView.dequeueReusableCell(with: .RightImageMessageBubble, for: indexPath) as! RightImageMessageBubble
                    ...
                    case .video where message.senderUid != LoggedInUser.uid:
                        if let videoMessage = message as? MediaMessage {
                            ...
                            case .thumbnailGeneration,.none:
                                let receiverCell = tableView.dequeueReusableCell(with: .LeftVideoMessageBubble, for: indexPath) as! LeftVideoMessageBubble
                    ...
                    case .video where message.senderUid == LoggedInUser.uid:
                        if let videoMessage = message as? MediaMessage {
                            ...
                            case .linkPreview, .smartReply, .messageTranslation, .profanityFilter,.sentimentAnalysis, .imageModeration, .reply: break
                            case .thumbnailGeneration,.none:
                                let senderCell = tableView.dequeueReusableCell(with: .RightVideoMessageBubble, for: indexPath) as! RightVideoMessageBubble
                    ...
                    case .audio where message.senderUid != LoggedInUser.uid:
                        ...
                        if let audioMessage = message as? MediaMessage {
                            let  receiverCell = tableView.dequeueReusableCell(with: .LeftAudioMessageBubble, for: indexPath) as! LeftAudioMessageBubble
                            receiverCell.audioMessage = audioMessage
                            return receiverCell
                        }
                    case .audio where message.senderUid == LoggedInUser.uid:
                        if let audioMessage = message as? MediaMessage {
                            let senderCell = tableView.dequeueReusableCell(with: .RightAudioMessageBubble, for: indexPath) as! RightAudioMessageBubble
                    ...
                    case .file where message.senderUid != LoggedInUser.uid:
                        if let fileMessage = message as? MediaMessage {
                            let  receiverCell = tableView.dequeueReusableCell(with: .LeftFileMessageBubble, for: indexPath) as! LeftFileMessageBubble
                    ...
                    case .file where message.senderUid == LoggedInUser.uid:
                        if let fileMessage = message as? MediaMessage {
                            let senderCell = tableView.dequeueReusableCell(with: .RightFileMessageBubble, for: indexPath) as! RightFileMessageBubble
                ...
                //  ActionMessage Cell
                let  actionMessageCell = tableView.dequeueReusableCell(with: .ActionMessageBubble, for: indexPath) as! ActionMessageBubble
                ...
                //  CustomMessage Cell
                let  receiverCell = tableView.dequeueReusableCell(with: .ActionMessageBubble, for: indexPath) as! ActionMessageBubble
...
extension CometChatMessageList : ChatViewInternalDelegate {
    ...
    public func didAttachmentButtonPressed() {
        ...
        let cameraAction: UIAlertAction = UIAlertAction(title: NSLocalizedString("Take a Photo", tableName: nil, bundle: CCPType.bundle, value: "", comment: ""), style: .default) { action -> Void in
        ...
        let photoLibraryAction: UIAlertAction = UIAlertAction(title: NSLocalizedString("PHOTO_&_VIDEO_LIBRARY", tableName: nil, bundle: CCPType.bundle, value: "", comment: ""), style: .default) { action -> Void in
        ...
        let documentAction: UIAlertAction = UIAlertAction(title: NSLocalizedString("DOCUMENT", tableName: nil, bundle: CCPType.bundle, value: "", comment: ""), style: .default) { action -> Void in
...
extension CometChatMessageList : CometChatMessageDelegate {
    ...
    private func appendNewMessage(message: BaseMessage) {
        playIncomingMessageSound(pausingCurrentAudio: true)
    ...
    public func onTextMessageReceived(textMessage: TextMessage) {
        DispatchQueue.main.async{ [weak self] in
            ...
            if let sender = textMessage.sender?.uid, let currentUser = strongSelf.currentUser?.uid {
                ...
                self?.playIncomingMessageOther(pausingCurrentAudio: true)
            }
        ...
        DispatchQueue.main.async{ [weak self] in
            ...
            if typingDetails.sender?.uid == strongSelf.currentUser?.uid && typingDetails.receiverType == .user {
                ...
                if let typingMetaData = typingDetails.metadata, let type = typingMetaData["type"] as? String ,let reaction = typingMetaData["reaction"] as? String {
                    ...
                    strongSelf.setupNavigationBar(withSubtitle: NSLocalizedString("TYPING", tableName: nil, bundle: CCPType.bundle, value: "", comment: ""))
                }
                ...
                if let typingMetaData = typingDetails.metadata, let type = typingMetaData["type"] as? String ,let reaction = typingMetaData["reaction"] as? String {
                    ...
                    if let user = typingDetails.sender?.name {
                        strongSelf.setupNavigationBar(withSubtitle: "\(String(describing: user)) " + NSLocalizedString("IS_TYPING", tableName: nil, bundle: CCPType.bundle, value: "", comment: ""))
                    }
            ...
            if typingDetails.sender?.uid == strongSelf.currentUser?.uid && typingDetails.receiverType == .user{
                ...
                if let typingMetaData = typingDetails.metadata, let type = typingMetaData["type"] as? String ,let reaction = typingMetaData["reaction"] as? String {
                    if type == "live_reaction" {
                        ...
                        strongSelf.setupNavigationBar(withSubtitle: NSLocalizedString("ONLINE", tableName: nil, bundle: CCPType.bundle, value: "", comment: ""))
                    }
    ...
    public func onUserOffline(user: User) {
        if user.uid == currentUser?.uid {
            if user.status == .offline {
                DispatchQueue.main.async {  [weak self] in
                    guard let strongSelf = self else { return }
                    strongSelf.setupNavigationBar(withSubtitle: NSLocalizedString("OFFLINE", tableName: nil, bundle: CCPType.bundle, value: "", comment: ""))
                }
            }
        }
    }
...
extension CometChatMessageList : SmartRepliesViewDelegate {
    ...
    func didSendButtonPressed(title: String) {
        ...
        case true:
            ...
            CometChat.sendTextMessage(message: textMessage!, onSuccess: { (message) in
                print("sendTextMessage onSuccess: \(String(describing: message.stringValue()))")
                self.playOutgoingMessageSound(pausingCurrentAudio: true)
                if let row = self.chatMessages[lastSection].firstIndex(where: {$0.muid == message.muid}) {
                    self.chatMessages[lastSection][row] = message
                }
                DispatchQueue.main.async{  [weak self] in
