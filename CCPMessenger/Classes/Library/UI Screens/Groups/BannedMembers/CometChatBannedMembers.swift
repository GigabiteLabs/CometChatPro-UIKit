...
extension CometChatBannedMembers: UITableViewDelegate , UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, contextMenuConfigurationForRowAt indexPath: IndexPath, point: CGPoint) -> UIContextMenuConfiguration? {
        ...
        return UIContextMenuConfiguration(identifier: nil, previewProvider: nil, actionProvider: { suggestedActions in
            ...
            let unbanMember = UIAction(title: NSLocalizedString("UNBAN_MEMBER", tableName: nil, bundle: CCPType.bundle, value: "", comment: ""), image: UIImage(systemName: "person.crop.circle"), attributes: .destructive) { action in
                if  let selectedCell = tableView.cellForRow(at: indexPath) as? MembersView {
                    if let member = selectedCell.member, let uid = member.uid, let guid = self.currentGroup?.guid {
                        ...
                        CometChat.unbanGroupMember(UID: uid, GUID: guid, onSuccess: { (sucess) in
                            ...
                            let message =  (member.name ?? "") + NSLocalizedString("UNBANNED_SUCCESSFULLY", tableName: nil, bundle: CCPType.bundle, value: "", comment: "") 
