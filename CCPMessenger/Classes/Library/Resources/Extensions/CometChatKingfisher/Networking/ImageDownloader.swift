...
public struct ImageLoadingResult {
    ...
    public let image: CFCrossPlatformImage
...
public struct DownloadTask {
    ...
    /// to a same `sessionTask`. This is an optimization in CometChatKingfisher to prevent multiple downloading task
    ...
    ///
    /// - Note:
    /// In CometChatKingfisher, there is an optimization to prevent starting another download task if the target URL is being
...
open class ImageDownloader {
    ...
    public init(name: String) {
        if name.isEmpty {
            fatalError("[CometChatKingfisher] You should specify a name for the downloader. "
    ...
    /// Downloads an image with a URL and option. Invoked internally by CometChatKingfisher. Subclasses must invoke super.
    ///
    /// - Parameters:
    ///   - url: Target URL.
    ///   - options: The options could control download behavior. See `CometChatKingfisherOptionsInfo`.
    ...
    open func downloadImage(
        ...
        completionHandler: ((Result<ImageLoadingResult, CometChatKingfisherError>) -> Void)? = nil) -> DownloadTask?
    {
        ...
        if let requestModifier = options.requestModifier {
            ...
            guard let r = requestModifier.modified(for: request) else {
                options.callbackQueue.execute {
                    completionHandler?(.failure(CometChatKingfisherError.requestError(reason: .emptyRequest)))
                }
                return nil
            }
            request = r
        ...
        guard let url = request.url, !url.absoluteString.isEmpty else {
            options.callbackQueue.execute {
                completionHandler?(.failure(CometChatKingfisherError.requestError(reason: .invalidURL(request: request))))
            }
            return nil
        }
        ...
        let onCompleted = completionHandler.map {
            block -> Delegate<Result<ImageLoadingResult, CometChatKingfisherError>, Void> in
            let delegate =  Delegate<Result<ImageLoadingResult, CometChatKingfisherError>, Void>()
